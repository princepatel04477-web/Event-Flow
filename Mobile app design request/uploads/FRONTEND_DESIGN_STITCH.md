...
## 1. Design concept — "the bahi-khata account ledger"

The app's metaphor is structural, not decorative: **this software accounts for
people**. Every guest who arrived must be accounted for on the way out — the
same promise an Indian account ledger (bahi-khata) makes with its ruled paper,
red margin rule, and figures that line up.

Rules that fall out of the metaphor — **do not break these**:

- **Paper is warm, never white.** `#f7f3ea` — paper, not a screen.
- **Alternating list bands** are a shade lighter than the paper (`#f1ecdf`),
  like ruled ledger lines. Lists are banded row by row.
- **Ink is near-black with a blue cast** (`#18222f`), never pure black.
- **Ledger red means ATTENTION and only that** — overdue, over capacity,
  unbalanced, destructive, unreachable. If red appears on a heading it stops
  meaning "look here". Never decorative.
- **Ledger green means COMPLETED and only that** — confirmed, delivered,
  balanced.
...
- **No page transitions, no scroll animations.** One 150ms fade is the only
  entrance; presses are 100ms colour shifts. Everything is fast because staff
  are in a corridor.
- **Dark mode follows the OS** — staff will not hunt for a theme toggle at
  11pm in a banquet hall. Dark is a "night ledger": warm near-black paper,
  preserved ink cast.
...
|---|---|---|---|
| paper (page background) | `#f7f3ea` | `#141a24` | Page background, list row base |
| paper-band (alternating row) | `#f1ecdf` | `#1a212c` | Zebra band on odd rows |
| surface (cards) | `#fbf9f2` | `#1a212c` | Cards, inputs, chips |
| surface-2 (nested fill) | `#efe9da` | `#232b38` | Secondary fills, hover, selected rows |

**Ink / text**

| Token | Light | Dark | Use |
|---|---|---|---|
| ink | `#18222f` | `#e8e6df` | Primary text; also the primary button fill |
...
- rule: `#18222f14` light / `#e8e6df14` dark (hairlines, skeleton fills)
- rule-strong: `#18222f26` light / `#e8e6df26` dark (borders, field outlines)

**Semantic colors** — the only two "brand" colors in the system:
...
**Supporting tones** (still constrained — used only for "in progress" and
generic info, never for the two ledger meanings):
- warning (amber, "in hand / in progress / caution"): `#b45309` light, `#fcd34d`-family dark; tint `#fef3c7` / dark `#3b2f12`-family
- info (blue, neutral information): `#0369a1`; tint `#e0f2fe`
- brand (interactive accent — selected tabs, selected filter chips, selected
  outcome cards): indigo `#4338ca` light / softened per-dark; fg `#ffffff`
- focus ring: `--ef-ring` = ink (`#18222f` / `#e8e6df`)
...
| Role | Stack | Sizes |
|---|---|---|
| Sans (body, controls) | `ui-sans-serif, system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", sans-serif` | 12px (xs, tertiary labels), 14px (sm, meta/secondary), **16px (base, body — never below; anything smaller makes iOS Safari zoom on focus)**, 20px (lg, body emphasis), 24px (xl), 28px (2xl, the one thing on screen) |
| Display (character face) | `ui-serif, Georgia, "Times New Roman", "Noto Serif", serif` | The one identifier on a row (family head name), EmptyState/ErrorState titles, SectionHead titles |
...
- Focus-visible is a 2px ink outline with 2px offset, everywhere.
...
- **Radii**: `rounded-xl` (12px) for cards, buttons, inputs, chips; `rounded-lg`
  (8px) for inner chips and small fills; `rounded-2xl` (16px) for outer card
  containers; `rounded-full` for pills/badges/stat icon chips.
- **Cards** sit on the paper with a `surface` background, `rule-strong` hairline
  border, and **no drop shadows** — the ledger has no floating chrome.
- **Buttons**: primary = solid ink fill with paper text (`bg-ink text-paper`);
  secondary = surface fill, strong-rule border, ink text; danger = solid
  ledger-red with paper text; ghost = transparent, ink text. Minimum heights:
  `md` 44px, `lg` 56px. Never below 44px — thumbs, gloves, rain.
