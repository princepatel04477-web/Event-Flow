...
export function RoomCreateForm({ eventId, hotelId, eventCode, hotelName }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      setError(res.error ?? 'Failed to create rooms.')
    }
    setSubmitting(false)
  }
      ...
      <div className="flex flex-col items-center gap-6 py-12 text-center">
        ...
        <div>
          ...
          <p className="mt-1 text-muted">
            {result.created} room{result.created === 1 ? '' : 's'} added
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      {mode === 'range' ? (
        <div className="flex flex-col gap-4">
          <label className="flex flex-col gap-1.5">
            <span className="text-sm font-medium text-fg">Room prefix (e.g. floor or block)</span>
            <Input value={prefix} onChange={e => setPrefix(e.target.value)} placeholder="e.g. 2 for rooms 201, 202…" />
          </label>
          <div className="grid grid-cols-2 gap-4">
            <label className="flex flex-col gap-1.5">
              <span className="text-sm font-medium text-fg">First number</span>
              <Input value={start} onChange={e => setStart(e.target.value)} type="number" min="1" required />
            </label>
            <label className="flex flex-col gap-1.5">
              <span className="text-sm font-medium text-fg">Last number</span>
              <Input value={end} onChange={e => setEnd(e.target.value)} type="number" min="1" required />
            </label>
          </div>
          <p className="text-xs text-muted">
            Preview: {prefix}{start.padStart(end.length, '0')} → {prefix}{end.padStart(end.length, '0')}
          </p>
        </div>
      ) : (
        <label className="flex flex-col gap-1.5">
      ...
      <label className="flex flex-col gap-1.5">
        <span className="text-sm font-medium text-fg">Floor</span>
        <Input value={floor} onChange={e => setFloor(e.target.value)} placeholder="e.g. 1, Ground" />
      </label>
