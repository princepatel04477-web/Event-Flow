...
export function RoomEditClient({ roomId, eventId, eventCode, hotelId, initial }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null)
    setSubmitting(true)
    const result = await updateRoom(roomId, eventId, {
      roomNumber, roomType: roomType || null, floor: floor || null,
      capacity: parseInt(capacity, 10) || 2, notes: notes || null, isBlocked,
    })
    if (result.ok) {
      router.push(`/admin/events/${eventCode}/hotels/${hotelId}`)
      router.refresh()
    } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      <label className="flex items-center gap-3 rounded-xl border border-rule p-3">
        ...
        <div>
          <p className="text-sm font-medium text-fg">Block this room</p>
          <p className="text-xs text-muted">Prevents new allocations until unblocked</p>
        </div>
      </label>
