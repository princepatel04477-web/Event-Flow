...
// ---------------------------------------------------------------------------
// Preview: substitute a real group's data into a template
// ---------------------------------------------------------------------------

export interface TemplatePreview {
  body: string
  /** Which variables have values and which are missing. */
  missing: string[]
}
...
export async function previewTemplate(
  ...
  const { data: group } = await supabase
    .from('guest_groups')
    .select('head_name, primary_mobile, side, rsvp_status')
    .eq('id', groupId)
    .eq('event_id', eventId)
    .maybeSingle()

  if (!group) return { error: 'Guest group not found.' }

  const vars = resolveVars(group as Record<string, unknown>)
  const body = (template.body as string).replace(
    /\{\{(\w+)\}\}/g,
    (_, key: string) => vars[key] ?? `{{${key}}}`,
  )

  const variables = (template.variables as string[]) ?? []
  const missing = variables.filter((v) => !(v in vars))
...
function resolveVars(
  ...
  const base: Record<string, string> = {
    head_name: (group.head_name as string) ?? '',
    event_name: '',
  }

  if (group.primary_mobile) {
    base.head_name = base.head_name || (group.primary_mobile as string)
  }
