...
- **Right:** The planned design for reversible writes (such as check-in toggles or guest room assignments), where the client updates local state immediately and would surface a non-blocking undo bar (per `docs/UX-RULES.md` R5, though as noted in §The Six States below, no `UndoBar` component exists yet under `src/`). In contrast, `src/app/(staff)/[eventCode]/hospitality/deliveries/[deliverableId]/DeliveryDetail.tsx` (lines 29–35, 71–79, 365, 390–399) handles an irreversible write: inserting a photo proof into `delivery_proofs`, where unconditional `app.block_mutation()` triggers (`CLAUDE.md §5.2`) prevent any update or delete. The screen blocks on the server round trip via a plain `<Button loading={phase.name === 'uploading'}>Confirm delivery</Button>` (line 365) rather than optimistic completion. Notably, there is no confirmation modal or dialog (the source comment at line 397 explicitly notes: "What follows it is a stub, not a confirmation dialog"), and the UI does not yet explicitly warn the user that the action is irreversible (the "can never be undone" phrasing exists only in code comments at line 392, not user-facing copy). Once committed, it renders a sealed stamp (`@utility seal-in`) and a read-only metadata stub.
- **Wrong:** `src/app/(staff)/[eventCode]/rsvp/status/[groupId]/RsvpLogForm.tsx` (lines 164–183), where logging an RSVP outcome (a fully reversible update to `guest_groups.rsvp_status`) halts user interaction by setting `saving = true` and blocking behind `await saveRsvpLog(...)` before showing the saved outcome, instead of immediately applying the status locally.
...
- **Wrong:** `src/app/(staff)/[eventCode]/rsvp/status/[groupId]/RsvpLogForm.tsx` lines 374–382: `<Button size="lg" fullWidth loading={saving} disabled={status === ''} onClick={handleSave}>Save outcome</Button>` (where line 377 sets `loading={saving}`), which disables the button and spins during the network round trip for a routine, reversible RSVP status update.
...
| **4. Content-plus-pending write** | A write mutation has been triggered. Existing content remains completely interactive while the write reconciles with the server in the background. | **NO COMPONENT YET** |
...
### The Missing Component: Content-Plus-Pending Write

**State 4 (Content-plus-pending write) has NO component yet in this repository.**

Currently, when a mutation occurs, the codebase either:
1. Locks the control and blocks the user behind `<Button loading>` (`src/components/ui/Button.tsx`), violating Rules T2 and T3, or
2. Updates state silently without providing any visible indicator that a background reconciliation with Supabase is underway.

To fulfill Rule T2, EventFlow needs a dedicated, non-blocking component (such as an `OptimisticRow` wrapper, `PendingWriteIndicator`, or subtle status badge) that keeps existing content visible and usable while displaying a discreet, non-blocking indication (e.g., a subtle accent pulse or pending chip) that the write is queued or reconciling with the server.
